...
- Apresentar, explicar e interpretar como as técnicas de Design Instrucional foram implementadas nos materiais didáticos específicos de Computação na EAD no Brasil.
...
### Lista de conteúdo por capítulo

Esse recurso consiste em apresentar um minisumário por capítulo, para que o aluno possa ver antecipadamente os conteúdos que serão abordados por ela, as vezes também apresenta-se através de uma lista de conceitos [@Freeman2005 p. 132].
...
Arquitetura         | Contempla as disciplinas de arquitetura de computadores ou introdução a computação ou informática.
...
O *string* final de busca elaborado para representar as disciplinas foi: “Algoritmo Programação Sistema Software Programa Dados Computação Informática Computador”. De forma similar, percebe-se que todas as disciplinas são contempladas pelo parâmetro de busca.
...
Durante a etapa de *download* de materiais todos os *resultados filtrados* foram baixados e renomeados -- com um pré-fixo correspondente a sua posição na planilha para facilitar as identificações.
...
As verificações dos conteúdo dos arquivos foram realizadas na próxima etapa.
...
Para facilitar a análise, foi utilizada uma escala ordinal para pontuar como as técnicas foram utilizadas. Para cada livro as técnicas serão pontuadas de 1 a 5, de acordo com a utilização da técnica e o quanto ela contemplada a obra:

Escala |  Contempla de forma suficiente
:-----:|  :-----------------------------:
1      |  A técnica não foi utilizada
2      |  Contempla até 25% da obra
3      |  Contempla de 26% a 50% da obra
4      |  Contempla de 51% a 75% da obra
5      |  Contempla de 76% a 100% da obra
...
A avaliação dos objetivos de aprendizagem apresenta as seguintes dificuldades: apresentação de metas no lugar de objetivos, o dimensionamento dos objetivos para o capítulo, objetivos apresentados por unidade (agrupamento de capítulos) e objetivos apresentados de forma textual.
...
*Somente os objetivos apresentados em formato de lista serão considerados*. Alguns livros apresentam os objetivos em um parágrafo de texto dentro de uma caixa, esses objetivos serão desconsiderados, pois geralmente contém *metas* e não costumam ser utilizados com o propósito de nortear a autoavaliação dos alunos. 
...
- Saudações
- Convites a reflexões e envolvimento
- Perguntas retóricas
...
*Todas as atividades presentes no texto* serão consideradas atividades de autoavaliação.

### Espaço para realizações das atividades

A verificação de espaços para realizações das atividades é relativamente fácil, basta procurar por linhas ou lacunas para o preenchimento nas atividades. 

*As atividades de marcar Verdadeiro (V) ou Falso (F) quase sempre proporcionam espaço para seu preenchimento*. Mesmo em um livro diagramado sem espaços para preenchimento das atividades, esse tipo de atividade permite naturalmente espaço para o aluno responder as atividades e devem ser contabilizados como tal.

*Os espaços deixados para anotações nos finais dos capítulos não devem ser confundidos com espaço para realizações de atividades*. Alguns materiais deixam espaços para anotações nos final do capítulos e quando as atividades também estão condensadas no final pode-se imaginar que eles foram reservados para as realizadações das atividades, no entanto nesses casos percebe-se que os espaços não são convidativos para as respostas, nem possuem o tamanho apropriado para as respostas, devendo portanto serem ignorados.


### Ilustrações Didáticas

As ilustrações serão consideradas didáticas somente quando estiverem associadasa ao texto, auxiliando a compreensão do conteúdo. Nessa categoria enquadram-se esquemas, fotos ou ilustrações sobre o conteúdo ou atividade apresentados, contribuindo para a compreensão ou ilustração do texto.
...
Fotos de pessoas ou personagem também serão ignoradas, quando o propósito for de apenas ilustrar a personalidade. Alguns textos apresentam fotografias de pessoas que estão sendo mencionadas, essas imagens não facilitam o entendimento do texto nem auxiliam no aprofundamento das discussões, sevem apenas para apresentar o personagem. Essa decisão foi tomada para diferenciar livros com ilustrações que facilitam o entendimento de livros que possuem fotografias. Por exemplo, um livro de filosofia que apresenta imagens dos filósofos é diferente de um livro que apresenta esquemas e ilustrações das teorias apresentadas, tornando o apredizado mais efetivo. Portanto imagens de personagens não serão consideradas ilustrações didáticas, e serão ignoradas durante a avaliação.

As ilustrações serão contabilizadas por página. Ao avaliar a utilização das ilustrações, podemos contabilizar as páginas ilustradas e verificar sua abrangência para a obra, não sendo necessário contabilizar individualmente cada ilustração, o que tornaria a avaliação mais custosa. De forma geral, nós temos uma sensação de que um livro é mais ilustrado do que o outro contando a quantidade de páginas que possuem ilustrações.

Códigos fontes não serão considerados ilustrações didáticas. Em livros de informática é comum encontrar códigos fontes, mas eles não serão considerados ilustrações, a não ser que hajam desenhos, tais como a ilustrações de mudança de valor de uma variável após uma atribuição, ou comparação de códigos, etc.

### Código com destaque da sintaxe

Essa técnica será contabilizada mesmo que o código seja apresentado como uma imagem. Alguns livros apresentam códigos fonte como imagens contendo os destaques das sintaxes, nesses casos eles erão considerados códigos com destaque da sintaxe e não serão contabilizados como ilustrações.

A avaliação dos códigos com destaque da sintaxe, similiar as ilustrações, também serão contabilizados por página.

### Glossário

A abrangência do glossário pode ser verificada por página ou contadas aos final do capítulo. Alguns livros apresentam as expressões do glossário ao longo do texto, próximo ao primeiro uso da palavra e outros apresentam uma seção de glossário por capítulo ou ainda apenas um glossário no final do livro. Independente do caso, a abrangência do glossário pode ser avaliada verificando a proporção e palavras por capítulo.

Quando a apresentação do glossário ocorre sem quebrar o fluxo, essa informação também deve ser contabilizada como mostrado a seguir.

###    Informações periféricas sem quebrar fluxo

As informações periféricas devem ser contabilizadas sempre que houverem caixas de texto sendo apresentadas sem quebrar o fluxo natural de leitura. As informações periféricas costumam apresentar conteúdos complementares, tais como curiosidades, aprofundamentos, e até mesmo compondo um glossário. No último caso, percebe-se que cada ocorrência de um glossário pode também corresponder uma ocorrênciade desta técnica, se ele também for apresentado sem quebrar o fluxo do texto.


### Resumos ou sínteses

Serão considerados resumos ou sínteses seções ou caixas apresentadas no final dos capítulos, ou parágrafos em finais de seções com o propósito de rever o que foi apresentado por elas.

No caso dos resumos apresentedos no final dos capítulos, espera-se que essa técnica seja empregada uma uníca vez por capítulo, abrangindo o capítulo por inteiro.

Em alguns desenhos instrucionais, os capítulos podem ser dividos em seções independentes, apresentando parágrafos de resumo ao final das seções. Nesses casos a abrangência do resumo contempla a seção apresentada, a abrangência da técnica em relação ao capítulo dependerá da presença dos resumos de todas as seções que o compõe.

### Referências a conteúdos multimídias

Nessa pesquisa somente vídeos, filmes, áudios, animações e simulações serão considerados conteúdos multimídias. As referências ao conteúdos multimídia portantos pode ser apresentados nos livros através de indicações, fornecendo os links dos conteúdos, ou simplesmente indicando o nome do filme por exemplo. Similar ao resumo, uma única ocorrência de referência de conteúdo multimídia contempla o capítulo inteiro.


### Lista de conteúdo por capítulo

Essa técnica será contabilizada sempre que presente um minisumário ou uma lista de conteúdos por capítulo. Quando presente a técnica contempla o capítulo inteiro.

Sumários devem ser ignorados. O sumário no início dos livros não corresponde a essa técnica, que é presente por capítulo e portanto não deverá ser contabilizados.


## Processo de aquisição dos dados

O processo de aquisição dos dados consistiu em avaliar, com o auxílio de um formulário, as técnicas empregadas nos livros e registrar as avaliações para análise posterior.

### Avaliação

Avaliar as abrangências de todas as técnias sobre a obra não é uma atividade trivial, pois requer perceber como todas elas foram utilizadas ao longo de toda obra. Por exemplo, é fácil perceber que um livro não possui ilustrações, basta folheá-lo rapidamente. No entanto, perceber se as ilustrações contidas em um livro são didáticas e como elas abrangem a obra iteira requer um olhar mais detalhado.

A avaliação de todas as técnicas presentes em um livro só pode realizada após a caracterização do uso de todas as técnicas presentes no livro. Para caracterizar o uso das técnicas na obra, pode-se utilizar medições por capítulos -- divisão natural dos livros. Com esse propóstio, um formulário foi elaborado para registrar as medições.

### Formulário de avaliação

O formulário foi de avaliação foi desenvolvido durante a execução do piloto. Inicialmente ele era digital e possuía campos reservados para o registro de utilização de todas as técnicas por capítulo. Mas depois percebemos que a verificação dos livros no computador e o preenchimento do formulário no mesmo meio tornava o processo demorado. Portanto o formulário passou a ser impresso e preenchido a lápis.

O formulário passou por diveras modificações quando o formato mudou para o impresso. No formato final do formulário impresso, cada página permite a avaliação de dois livros, com espaços que facilitam as análises das obras por capítulos ou grupos de capítulos. Um espaço maior foi reservado para as técnicas mais utilizadas.

A ordem das técnicas no formulário foram otimizadas para facilitar as medições. Os campos de *Objetivos de aprendizagem* e *Resumo* foram posicionadas no início e final do formulário respectivamente, o que corresponde com as posições esperadas nos capítulos. A técnica *linguagem dialógica* ocupa a segunda posição, que pode ter seu correspondente nos capítulos, caso eles iniciem com dialógos. Em seguida a técnica de ilustração, que costuma ser bastante frequente. As técnicas de *Glossário, Fluxo alternativo e Referências multimídia* podem ser uma sequência natural nos livros, e sua aproximidade permite registrar o uso concorrente das técnicas, como por exemplo um glossário apresentado através de fluxo alternativo. E próximo do fim, as técnicas de *Atividades* e *Espaço para realizações das atividades* foram contempladas próximas, facilitando seu preenchimento.

O formulário também possibilita registro qualitativo de informações. Para possibilitar triangulação na pesquisa, o formulário possibilita registro de informações adicionais que dificilmente seriam consideradas em uma pesquisa puramente quantitativa, como por exemplos: a indicação de que um livro não aparentava está diagramado ou que possui layout de revista (múltiplas colunas).

### Registros das avaliações

As avaliações das técnicas eram registradas primeiramente no formulário e posteriormente digitadas em uma planilha. Após a mensuração das técnicas, suas avaliações eram realizadas e registradas no formulário, que também recebia uma numeração para identificação e arquivamento. Posteriormente as informações eram registradas numa planilha, os formulários digitalizados e arquivados.


<!--  

Mostrar o diretório contendo as planilhas digitalizadas.
...
- No formulário, os campos das técnicas foram marcados com uma inicial para evitar que o avaliador erre o local de registro da técnica em medição.

é fácil confundir a medição

de um formulário impresso agilizaria o processo, poi




As medições das técnicas foram realizadas através de identificações de usos por capítulos.
