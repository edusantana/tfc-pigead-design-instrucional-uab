...
Pendente: Trocar o nós por você não torna o texto dialógico. Exemplo: Neste primeiro capítulo *você* verá como realizar uma operação de exponenciação.
...
### Definindo o Corpus de Computação
...
Essa consulta, realizada em Janeiro de 2015, retornou 541 resultados, sendo a maioria resultados irrelevantes, sendo necessário um processo para sua remoção.
...
A remoção dos resultados irrelevantes foi através da identificação de palavras que os indentificassem nas linhas da planilha. Para essa pesquisa, as seguintes palavras identificavam esses resultados: ``vídeo, video, tutorial, aula, saúde, saude, infográfico, educac, animação, podcast, ética, Instalaç e Bioinfo''. A exclusão foi realizada através do seguinte comando, que cria uma nova planilha com os resultados filtrados:
...
Esse processo removeu 375 linhas (69%), deixando a nova planilha ("filtrados.csv") com apenas 166 resultados (31%). Para manter o cunho ético da pesquisa, as instituições e os cursos de origem dos materiais foram ofuscadas.
...
Para elaborar e calibrar os instrumentos de coleta de dados e avaliação, uma piloto foi conduzido com dois livros de cada instituição (sorteados aleatoriamente) para serem analisados.
...
1      |  Não contempla
2      |  Até 25% da obra
3      |  De 25% a 50% da obra
4      |  De 50% a 75% da obra
5      |  De 75% a 100% da obra
